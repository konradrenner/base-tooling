...
* direnv (+ nix-direnv)
* git (incl. user name + email, configured at install time)
...
NOTE: `--git-name` and `--git-email` are required and are passed as environment variables to Nix at build time. They are never stored in the repository.
