...
* direnv (+ nix-direnv)
* git (user name configured once at install time via `~/.gitconfig-identity`; email is set per repo)
...
[source,bash]
----
curl -fsSL https://raw.githubusercontent.com/konradrenner/base-tooling/main/install.sh | bash -s -- \
  --user <ABC> \
  --git-name "Your Name"
----
...
. Write `~/.gitconfig-identity` with your git name (skipped if already present)
. Apply configuration using:
.. macOS: +darwinConfigurations.default+
.. Linux: +homeConfigurations.<ABC>@linux+

NOTE: `--git-name` is only required on the first run. `~/.gitconfig-identity` is a local file, never stored in this repository. Git email is intentionally not set globally — configure it per repo with `git config --local user.email "you@example.com"`.
...
To link:https://nix.dev/manual/nix/2.26/package-management/garbage-collection[remove old or unused packages], you can just enter the following command, so that you free some space and get rid of unused or old stuff:
